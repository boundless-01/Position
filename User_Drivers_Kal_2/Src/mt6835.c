...
uint32_t MT6835_ReadAngle_21bit(SPI_HandleTypeDef *hspi)//从MT6835传感器读取21位角度数据的函数
{
	uint16_t data_send = MT6835_CMD_CONTINUOUS_READ | MT6835_READ_REG_ANGLE_3;
	//触发连续读取和指定读取寄存器（角度寄存器3）的命令
	
	uint16_t data_rev[3];
	//包含3个16位元素的数组，用于存储从SPI接收到的数据
	
	uint32_t Angle_21bit;
	
	if(hspi == ENCODER_Y_SPI)
	{
		ENCODER_Y_CS_ENABLE;//使能（激活）Y轴编码器的片选信号
	}
	else if(hspi == ENCODER_X_SPI)
	{
		ENCODER_X_CS_ENABLE;//使能（激活）X轴编码器的片选信号
	}

	HAL_SPI_TransmitReceive(hspi,(unsigned char *)&data_send,(unsigned char *)&data_rev,3,10);
	//发送 data_send 命令并接收3个字节的数据到 data_rev 数组中
	
	Angle_21bit = (data_rev[1]<<5)|(data_rev[2]>>11);

	if(hspi == ENCODER_Y_SPI)
	{
		ENCODER_Y_CS_DISABLE;//关闭Y轴编码器的片选信号
	}
	else if(hspi == ENCODER_X_SPI)
	{
		ENCODER_X_CS_DISABLE;//关闭X轴编码器的片选信号
	}	
...
{
	uint32_t Angle_21bit;
	Angle_21bit = 	MT6835_ReadAngle_21bit(ENCODER_X_SPI);//从MT6835传感器读取21位角度数据的函数
...
{
	uint32_t Angle_21bit;
	Angle_21bit = 	MT6835_ReadAngle_21bit(ENCODER_Y_SPI);//从MT6835传感器读取21位角度数据的函数
	uint32_t Data = Angle_21bit & 0x1FFFFF;
	return Data;
}
