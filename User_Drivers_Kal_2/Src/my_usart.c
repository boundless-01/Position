...
void DataSend(void)
{
    ...
    valSend.val=(float)psPara.rotatedPosX_robort;//DataSend->UpdateTalkInfo->RotateLevel1->RotatePos2Need中得到
    memcpy(tdata+2,valSend.data,4);
    valSend.val=(float)psPara.rotatedVelX_robort;//DataSend->UpdateTalkInfo中得到
    ...
    valSend.val=(float)psPara.imuPara.imuResult_Angle[0];//UpdateAngleByPixhawk中得到
...
{
			UART6_DMA_Out((uint8_t* )"%f,%f,",psPara.rotatedPosX_robort,psPara.rotatedPosY_robort);
	UART6_DMA_Out((uint8_t* )"%f",psPara.imuPara.imuResult_Angle[2]);
	UART6_DMA_Out((uint8_t* )"%f\t",fabs(psPara.Kalmodelength - psPara.Modelength));
	UART6_DMA_Out((uint8_t* )"%d\t",psPara.ifadjustR);
//			UART6_DMA_Out((uint8_t* )"VelXY	%f\t%f\t\t",psPara.rotatedVelX_robort,psPara.rotatedVelY_robort);			
//			UART6_DMA_Out((uint8_t* )"reangle\t%f\t%f\t%f",psPara.imuPara.imuResult_Angle[0],psPara.imuPara.imuResult_Angle[1],psPara.imuPara.imuResult_Angle[2]);
//			UART6_DMA_Out((uint8_t* )"gyro %f\t%f\t%f\t",psPara.imuPara.imuGyro_Real[0],psPara.imuPara.imuGyro_Real[1],psPara.imuPara.imuGyro_Real[2]);
...
//			UART6_DMA_Out((uint8_t* )"encoderS  %f\t%f\t\t",psPara.ePara.encoderSum[0],psPara.ePara.encoderSum[1]);
//			UART6_DMA_Out((uint8_t* )"encoderM  %f\t%f\t",psPara.ePara.encoderMileage[0],psPara.ePara.encoderMileage[1]);
//			UART6_DMA_Out((uint8_t* )"time %d",psPara.timeCounter);
