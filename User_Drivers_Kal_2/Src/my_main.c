...
void main_loop(void)
{
	...
	while(1)
	{
		if(GetOneReadTimeFlag()) //1ms
		{
			//IMU数据采样
			IMU_DataSampling();
			//数据imu..SumRaw：三轴角速度，三轴加速度，due和uno的温度值（两个微控制器板）
		}
...
void PsPallInit(void)
{
	...
	psPara.oPara.ROTATE_ANGLE=-135.f;//定位系统坐标系->机体坐标系旋转角度
	...
	psPara.oPara.ORIGINYAW = 45.0f;//设置初始偏航角为 -45 度(没用)
	
	psPara.oPara.INSTALLYAW = 0.0f;//设置安装偏航角为 0 度(没用)
	
	psPara.oPara.CODEWHEELANGLEERR_YAW = 0.0f;//设置编码器轮偏航角误差为 0 度
	
	psPara.oPara.CODEWHEELANGLEERR_PITCH = 0.0f;//设置编码器轮俯仰角误差为 0 度
	
	psPara.oPara.ORIGINAL_POSX = 0.0f;//设置原始X轴位置为 -1067.159
	psPara.oPara.ORIGINAL_POSY = 0.0f;//设置原始Y轴位置为 532.844
	...
	psPara.imuPara.quaternion[0] = 1.f;
