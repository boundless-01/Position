...
void IMU_DataHandling(void)
{
	...
	//将原始数据转换为标准单位制，并取平均值
	scha63x_real_data_crc.acc_x =  imuAccSumRaw[0] / (4905.f * RUN_PERIOD);//4905.f是陀螺仪灵敏度
	scha63x_real_data_crc.acc_y =  imuAccSumRaw[1] / (4905.f * RUN_PERIOD);
	scha63x_real_data_crc.acc_z =  imuAccSumRaw[2] / (4905.f * RUN_PERIOD);
			
	scha63x_real_data_crc.gyro_x =  imuGyroSumRaw[0] / (80.f * RUN_PERIOD);//80.f是加速度计灵敏度
