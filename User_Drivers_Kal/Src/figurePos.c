...
//旋转矩阵{cos,sin,-sin,cos}：顺时针———注意：zangle以逆时针为正，即存在相对关系
	delPos[0]=(cos(zangle*0.017453292519943)*real[0]+sin(zangle*0.017453292519943)*real[1]);
	//0.017453292519943 是 PI/180
	delPos[1]=(-sin(zangle*0.017453292519943)*real[0]+cos(zangle*0.017453292519943)*real[1]);
	/*
	逆时针：
	x1=xcos(β)-ysin(β);
	y1=ycos(β)+xsin(β);
